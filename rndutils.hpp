...
// clang-format off
  ...
  // CLang
    ...
    #include <intrin.h>
    #define RNDUTILS_MSB_U32(x) (x ? 32 - _lzcnt_u32(x) : 0)
    #define RNDUTILS_POP_COUNT(x) __popcnt(x)
    #if defined(_M_X64)
      #define RNDUTILS_MSB_U64(x) (x ? 64 - _lzcnt_u64(x) : 0)
    #endif
  #endif
  #if !defined(RNDUTILS_CPU_TIMESTAMP_COUNTER)
...
namespace detail {
  ...
  struct debruijn_bitscanreverse
  {
    ...
    static constexpr int bsr(uint32_t x)
    {
      x |= x >> 1;
      x |= x >> 2;
      x |= x >> 4;
  ...
  inline int msb_uint(uint8_t x)
  {
    return static_cast<int>(RNDUTILS_MSB_U32(uint32_t(x)));
  }


  inline int msb_uint(uint16_t x)
  {
    return static_cast<int>(RNDUTILS_MSB_U32(uint32_t(x)));
  }


  inline int msb_uint(uint32_t x)
  {
    return static_cast<int>(RNDUTILS_MSB_U32(x));
  }


  inline int msb_uint(uint64_t x)
  {
    return static_cast<int>(RNDUTILS_MSB_U64(x));
  }
  ...
  inline auto do_generate_canonical(typename GCB::urng_t& rgen)
  ...
  inline auto make_high_entropy_seed_array() -> std::array<uint64_t, 8>
  {
    ...
    auto e0 = static_cast<uint64_t>(RNDUTILS_CPU_TIMESTAMP_COUNTER);
    // the classic: time, advertised with nano-second resolution.
    auto e1 = static_cast<uint64_t>(std::chrono::high_resolution_clock::now().time_since_epoch().count());
    // with some luck, this is non-deterministic: TRNG
    auto e2 = static_cast<uint64_t>(std::random_device{}());
    // different between invocations from different threads within one app: thread-id
    auto tid = std::this_thread::get_id();
    auto e3 = static_cast<uint64_t>(std::hash<decltype(tid)>()(tid));
    // different between installations: compile time macros
    auto e4 = static_cast<uint64_t>(std::hash<const char*>()(__DATE__ __TIME__ __FILE__));
    // different between platfroms: typeid
    auto e5 = static_cast<uint64_t>(typeid(make_high_entropy_seed_array).hash_code());
    // likely different between runs, invocations and platforms: address of local
    auto e6 = static_cast<uint64_t>(uintptr_t(&e0));
    // likely different between invocations: malloc address
    auto tmp = std::unique_ptr<unsigned>(new unsigned);
    auto e7 = static_cast<uint64_t>(uintptr_t(tmp.get()));
  ...
  inline auto make_low_entropy_seed_array() -> std::array<uint64_t, 8>
  {
...
public:
  ...
  {
    for (unsigned long long i = 0; i < z; ++z) this->operator()();
  }
...
public:
  ...
  friend bool operator==(engine_type const& lhs, engine_type const& rhs)
  {
    return 0 == std::memcmp(lhs.state_, rhs.state_, sizeof(state_));
  }
...
{
  auto seed_array = detail::make_high_entropy_seed_array();
...
// with the behaviour of std::discrete_distribution in the
